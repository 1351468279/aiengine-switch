...
安装器每次只配置一个客户端。如果只检测到一个客户端，会直接选择它；如果 Claude Code 和 Codex 都已安装，会要求选择本次配置哪一个。确认后在本机隐藏输入密钥，并通过 `https://modelapi.aiaiaiaiai.cloud/v1/models` 只验证该客户端需要的模型。

Claude 与 Codex 使用不同权限的密钥时，请分别运行两次安装命令，第一次选择一个客户端，完成后再次运行并选择另一个客户端。两份配置和密钥会独立保存，互不覆盖。
...
# 在线安装时明确选择 Codex；Claude 使用 --tools claude
